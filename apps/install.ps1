...
function Get-Spicetify {
  ...
  begin {
    ...
    if ($v) {
      ...
      # --- CHANGED: pakai jsDelivr data API, bukan api.github.com (menghindari rate limit) ---
      Write-Host -Object 'Fetching the latest spicetify version (via jsDelivr)...' -NoNewline
      try {
        $pkgData = Invoke-RestMethod -Uri 'https://data.jsdelivr.com/v1/packages/gh/spicetify/cli'
        $targetVersion = $pkgData.tags.latest
        if (-not $targetVersion) {
          # fallback: ambil versi pertama dari daftar versions kalau tag 'latest' tidak ada
          $targetVersion = $pkgData.versions[0].version
        }
      }
      catch {
        Write-Unsuccess
        Write-Warning -Message "Gagal mengambil versi terbaru dari jsDelivr. Coba lagi nanti atau tentukan versi manual dengan parameter -v, contoh: -v 2.36.4"
        Pause
        exit
      }
      Write-Success
...
if ($choice -eq 1) {
  ...
  # --- CHANGED: pakai jsDelivr CDN, bukan raw.githubusercontent.com ---
  $Parameters = @{
    Uri             = 'https://cdn.jsdelivr.net/gh/spicetify/spicetify-marketplace@main/resources/install.ps1'
